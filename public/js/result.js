...
function hideDay(day) {
  const layerNames = ['points', 'polylines', 'partitionsWorkDay'];
  if (!layersStore[day]) {
    layersStore[day] = [];
  }
  Object.keys(layers).forEach(vehicle => {
    layerNames.forEach(layerName => {
      const mainLayer = layers[vehicle][layerName].getLayers()[0];
...
function vehicleToGeojson(vehicle) {
  ...
  L.geoJSON({ type: "FeatureCollection", features: vehicle.points }, {
    pointToLayer: function(geoJsonPoint, latlng) {
      if (geoJsonPoint.properties.day && geoJsonPoint.properties.work_day) {
        const day = (geoJsonPoint.properties.day - 1) % 7;
        if (!dayMap[day]) {
          dayMap[day] = geoJsonPoint.properties.work_day;
        }
      }
